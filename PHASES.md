...
| 7 — Map it back to a real ML platform (stretch) | 🟡 draft written — kept private, not in this repo |
...
**Model inputs.** Every Avazu column except `click` (the label) and `id` (a row identifier, not a feature) goes in, mapped onto a user/ad/context grouping, the way an ad server organizes them:
...
| **User/device** | `device_id`, `device_ip`, `device_model`, `device_type`, `device_conn_type` | worth a deliberate call-out: `device_id`/`device_ip` are near-unique pseudo-identifiers, not real behavioral features — hash them into a small bucket space rather than one-hot encoding directly, or drop them and note in your write-up that real user features (behavioral aggregates from a feature store) would replace raw IDs entirely |
| **Anonymized (mixed)** | `C14`, `C17`–`C21` | undisclosed semantics; treat as categorical and encode the same way as the rest |

All of these get encoded into a single numeric feature vector per row; that vector is the actual input to `LogisticRegression` — there's no separate "user model" or "ad model," just one flat vector combining all three groups, which is the simplest version of the "cross features" idea (in a fuller system, interactions between user and ad groups would be engineered explicitly rather than left for logistic regression to find linearly).
...
- [x] 5. You can articulate, in your own words, what SageMaker did on your behalf (provisioning, pulling your code into its container, running it, capturing `/opt/ml/model` to S3) vs. what your script owns (all the actual ML logic) — that split is the platform/team boundary: the platform runs it, your team owns what runs.
...
- [x] Optional stretch: register it in the SageMaker Model Registry (a light touch — this is the closest AWS analog to "versioned model artifact gated by an eval report," which is how a mature CI/CD pipeline treats models).
...
**Goal:** the other offline pattern — batch prediction without a persistent endpoint, closer to how a platform would run scheduled scoring jobs.
...
## Phase 7 (stretch) — Map it back to a real ML platform ⬜

**Goal:** convert hands-on AWS experience into vocabulary for a specific organization's ML platform.

- [x] Write yourself a short one-pager: which piece of what you just did maps to the platform's offline half (training), which maps to the model-registry-in-CI/CD concept, and — importantly — where the mapping *breaks down* at the serving step (SageMaker endpoint vs. Kubernetes-based serving).
- [ ] This becomes a good artifact to sanity-check with the platform team early on.

*The write-up itself is organization-specific and is kept out of this repo — see `private/` (git-ignored).*
...
- [ ] 5. You can articulate why, in an org with a platform team, you probably wouldn't write this Terraform at all — the internal ML platform *is* the abstraction over it, and the platform team owns the IaC while your team owns model code and eval gates. Knowing where that seam is, is what lets you tell "our model is broken" from "the platform is broken."

**Note on ROI.** This phase teaches what the *platform* team owns, not what *your* team owns. That's real but second-order if you own models rather than infrastructure — model code, eval gates, and training/serving parity are the surface area that matters there. Hence: bottom of the stretch list, first to cut.
